...
//    return STATE.remaining_tries <= 0;
    return 1;//reter: assume remaining_tries=1
}

int8_t ctap_device_boot_locked()
{
//    return PIN_BOOT_ATTEMPTS_LEFT <= 0;
    return 1;//reter: assume PIN_BOOT_ATTEMPTS_LEFT=1
}
...
void LOG_HEX(uint32_t tag, uint8_t * data, int length)
{
 ...
 uint8_t test_ctap_request(uint8_t * pkt_raw, int length, CTAP_RESPONSE * resp)
{
    CborEncoder encoder;
    ...
    printf("cbor req: "); dump_hex1(TAG_DUMP, pkt_raw, length);
    switch(cmd)
    {
...
//            printf("make_credential time: %d ms\n", timestamp());
//
...
//            printf("get_assertion time: %d ms\n", timestamp());
//
//            resp->length = cbor_encoder_get_buffer_size(&encoder, buf);
//
//            printf("cbor [%d]: \n",  resp->length);
...
//            else
//            {
//                printf("unwanted GET_NEXT_ASSERTION.  lastcmd == 0x%02x\n", getAssertionState.lastcmd);
//                status = CTAP2_ERR_NOT_ALLOWED;
//            }
            break;
        case CTAP_CBOR_CRED_MGMT:
        case CTAP_CBOR_CRED_MGMT_PRE:
            printf("CTAP_CBOR_CRED_MGMT_PRE\n");
...
//            dump_hex1(TAG_DUMP,buf, resp->length);
        ...
        default:
            ...
            printf("error, invalid cmd: 0x%02x\n", cmd);
    }

done:
//reter debug    device_set_status(CTAPHID_STATUS_IDLE);
//reter debug    getAssertionState.lastcmd = cmd;

//reter debug    if (status != CTAP1_ERR_SUCCESS)
//reter debug    {
//reter debug        resp->length = 0;
//reter debug    }

    printf("cbor output structure: %d bytes.  Return 0x%02x\n", resp->length, status);
...
int test_json2cbor(int argc, char **argv)
{
    printf("\n ----------------test_json2cbor ----------------- enter\n");
...
void test_cbor(int argc, char **argv)
{
    ...
    //cbor json2cbor.c int main(int argc, char **argv)
    test_json2cbor(argc, argv);

    //cbor ctap.c uint8_t ctap_request(uint8_t * pkt_raw, int length, CTAP_RESPONSE * resp)
    int len = 0;
    CTAP_RESPONSE ctap_resp;
    test_ctap_request(ctap_buffer, len, &ctap_resp);
